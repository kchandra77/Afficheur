...
bool Afficheur::changerIdentifiant( unsigned int identifiant)
{
    m_erreur = AUCUNE;
    char _reponse[255];
...
bool Afficheur::ouvrirPortSerie(string nomPortSerie)
{
...
bool Afficheur::fermerPortSerie()
{
    return m_portSerie.fermer();
}

bool Afficheur::setNombrePages(int nombrePages)
{
    // On dit Ã  l'afficheur le nombre de page à afficher : 1 page dans cette version
    char _trameConfig[255] = "<TA>00010100009912302359A";
    char _reponse[10];
    for(int i=1 ; i<=nombrePages ; i++)
    {
        sprintf(_trameConfig,"%s%c", _trameConfig, (char)('A'-1 + nombrePages) );
    }
    char _message[255];
...
bool Afficheur::envoyerLigne(Ligne ligne)
{
    stringstream strame;
    strame << "<ID" << setfill('0') << setw(2) << setiosflags(ios_base::uppercase) << m_identifiant << ">" << ligne.getPaquetDonnees() << setfill('0') << setw(2) << setiosflags(ios_base::uppercase) << hex << (unsigned short)calculerChecksum(ligne.getPaquetDonnees()) << "<E>";
    return envoyerTrame(strame.str());
}


bool Afficheur::envoyerTrame(string trame)
{
...
bool Afficheur::envoyerTrame(const char * trame, int longueurTrame)
{
...
bool Afficheur::envoyerTrameEnUDP(string trame)
{
    if(nbTrameUDP == 0)
    {
		#ifdef _WIN32 || _WIN64
...
bool Afficheur::envoyerTrameEnUDP(const char * trame, int longueurTrame)
{
    string str(trame,longueurTrame);
...
bool Afficheur::envoyerLigneEnUDP(Ligne ligne)
{
    stringstream strame;
    strame << "<ID" << setfill('0') << setw(2) << setiosflags(ios_base::uppercase) << m_identifiant << ">" << ligne.getPaquetDonnees() << setfill('0') << setw(2) << setiosflags(ios_base::uppercase) << hex << (unsigned short)calculerChecksum(ligne.getPaquetDonnees()) << "<E>";
...
/*unsigned char Afficheur::calculerChecksum(const char * message) const
{
    // Calcul du CS
...
unsigned char Afficheur::calculerChecksum(string message) const
{
    // Calcul du CS
    unsigned char checksum = 0;
...
string Afficheur::descriptionErreur() const
{
